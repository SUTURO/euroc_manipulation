...
void goal_call_back(Server::GoalHandle gh)
{
    ...
    const unsigned int nr_lwr_joints = move_along_joint_path_srv.request.joint_names.size();
    ...
    for (unsigned int i = 2; i < nr_lwr_joints; ++i)
    {
...
{
    ros::init(argc, argv, "arm_base_controller");
    ros::NodeHandle n;
    ...
    Server arm_base_server(n, "/arm_base_controller/follow_joint_trajectory", false   );

    arm_base_server.registerCancelCallback(&cancel_call_back);
    arm_base_server.registerGoalCallback(&goal_call_back);
